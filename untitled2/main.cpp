...
const char* duze[8] = {"", " tysiac.", "tyciecy.", " tysiace.", " milion.", "milionów", "miliony"," miliard."};
    ...
    while (liczba>0)
    {
        ...
        if ((j==0)&&(liczba%100!=0 || koncowka!=0))
            if (koncowka > 1 && koncowka < 5)
                prepend(slownie, duze[2]);
            if (koncowka == 1)
                prepend(slownie, duze[1]);
            if (koncowka > 5)
                prepend(slownie, duze[2]);
            else
                prepend(slownie, duze[3]);
        if ((j==0)&&(liczba%10!=1))
            prepend(slownie, jednosci[koncowka]);
        if ((j==0)&&(liczba%10==1))
        {
